...
// #include "cfg_ezmodbus.h" // Supprimé car le fichier cfg_ezmodbus.h a été supprimé par l'utilisateur

// Toujours compiler ce code car il est dans un contexte IDF implicite
...
public:
    ...
    // Constantes internes pour le driver UART
    static constexpr int MAX_TOUT_THRESH = 102;
    static constexpr int DRIVER_RX_BUFFER_SIZE = 512; 
    static constexpr int DRIVER_TX_BUFFER_SIZE = 256; // Mettre à 0 pour un TX bloquant sans buffer driver
    static constexpr int DRIVER_EVENT_QUEUE_SIZE = 20; // Sera utilisé en interne
    ...
    // begin() applique la configuration et démarre le driver.
    ...
    // write retourne le nombre d'octets effectivement transmis et confirmés par l'UART.
    // Retourne 0 en cas d'échec de transmission physique complète.
    size_t write(const uint8_t* buf, size_t size); 
    int available() const;
    esp_err_t flush_input(); // Pour vider le buffer RX

    uint32_t getBaudrate() const { return _baud_rate; }
    // setBaudrate reconfigure le baudrate. Doit être appelé après begin() si on change dynamiquement.
    ...
    // Nouvelles méthodes pour mieux encapsuler les fonctionnalités RS485
...
private:
    ...
    gpio_num_t _pin_rts_de;  // Keep as gpio_num_t internally
    uint32_t _baud_rate;
    uint32_t _config_flags;
    int _pin_tx;
    int _pin_rx;
    QueueHandle_t _internal_event_queue_handle = nullptr; // Queue interne gérée par EzmUart
    bool _is_driver_installed = false;
    uart_config_t _current_hw_config; // Pour stocker la config appliquée à l'HW pour setBaudrate
};
