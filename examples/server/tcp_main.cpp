...
// TCP server for Modbus TCP
TCP tcpServer(MODBUS_PORT);

// Modbus TCP interface and server
...
void setup() {
    ...
    Serial.begin(115200);
    Serial.println("\nEZModbus Greenhouse TCP Server Example");
    ...
    server.addRegister({
        ...
        .writeCb = [](uint16_t value, const ModbusRegister& ctx) {
            ...
            if (currentTime - greenhouse.lastSetpointUpdateTime >= 1000) {  // Check every second
                ...
                int16_t requestedChange = abs((int16_t)value - (int16_t)greenhouse.lastTempSetpoint);
    ...
    server.addRegister({
        ...
        .writeCb = [](uint16_t value, const ModbusRegister& ctx) {
            ...
            if (currentTime - greenhouse.lastSetpointUpdateTime >= 1000) {  // Check every second
                ...
                int16_t requestedChange = abs((int16_t)value - (int16_t)greenhouse.lastHumidSetpoint);
